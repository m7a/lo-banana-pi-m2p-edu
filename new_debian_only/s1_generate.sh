...
if ! [ -f "$wd/u-boot-sunxi-with-spl.bin" ]; then
	...
	echo "logfile=$wd/download-uboot.txt"
	cp -r "$scriptroot/download-uboot" "$wd"
	cd "$wd/download-uboot"
	ant download > "$wd/download-uboot.txt" 2>&1
	mv "$wd/download-uboot/u-boot-sunxi-with-spl.bin" "$wd"
fi
...
mmdebstrap --architectures=armhf --components=main,contrib,non-free \
	--variant=important --include=mdvl-banana-pi-m2-plus-edu-root \
	--mode=unshare \
	> "$wd/fsroot.tar" <<EOF
